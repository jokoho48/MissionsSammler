#define ISSTRING(VAR) (VAR isEqualType "")
#define ISFLOAT(VAR) (VAR isEqualType 0)
#define dISARRAY(VAR) (VAR isEqualType [])
...
if !(dISARRAY(_spawnP)) then {
    if (ISSTRING(_spawnP)) then {
        _spawnP = getMarkerPos _spawnP;
    } else {
        _spawnP = getPos _spawnP;
    };
};
_spawnP set [2, 0];
...
if (ISSTRING(_unitsA select 0)) then {
    _grp1 = [_spawnP, _side, _unitsA, [], [], _spawnSkill] call BIS_fnc_spawnGroup;
} else {
...
switch (toUpper _handler) do {
    ...
    case ("CBA_DEFEND"): {
        ...
        [_grp1, _target, _Settings] call CBA_fnc_taskDefend;
    };
    case ("CBA_PATROL"): {
        if !(ISFLOAT(_Settings)) then { _Settings = 120; hint "CBA Need A Number as Setting!";};
        [_grp1, _target, _Settings] call CBA_fnc_taskPatrol;
    };
    case ("UPSMON"): {
